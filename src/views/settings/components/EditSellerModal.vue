<template>
  <dialog id="dialogCreateCategory" ref="dialogEditSellerRef" class="modal" @keydown.escape="closeCreateCategoryModal">
  ...
  <form @submit.prevent="handleSubmit" class="w-full space-y-4">
        ...
        <fieldset class="form-control w-full">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
            <label v-for="opt in permissionsOptions" :key="opt.value" class="cursor-pointer inline-flex items-center space-x-2">
              <input type="checkbox" :value="opt.value" v-model="selectedPermissions" class="checkbox" />
              <span class="text-sm">{{ opt.label }}</span>
            </label>
          </div>
          <p class="text-xs text-muted mt-2">Seleccione uno o más permisos para el usuario/vendedor {{ formData.permissions }}.</p>
        </fieldset>
...
// Lista de permisos disponibles (números del 1 al 9)
...
const dialogEditSellerRef = ref()
const formData = reactive({
  name: '',
  pin: '',
  // ahora es un número entero que concatenará los dígitos seleccionados (ej. 13)
  permissions: 0 as number,
  status: 'active',
})

// Mantener un array interno para las checkboxes y sincronizarlo a formData.permissions
const selectedPermissions = ref<number[]>([])

// Si cambia la selección, actualizamos el número concatenado (ordenado ascendente)
watch(selectedPermissions, (val) => {
  ...
  // Concatenar como string y convertir a número para almacenar
...
const closeCreateCategoryModal = () => {
  dialogEditSellerRef.value.close()
  show.value = false
}
...
const openCreateCategoryModal = () => {
  ...
  formData.permissions = 0
  dialogEditSellerRef.value.showModal()
}

const handleSubmit = () => {
  // Validación mínima: al menos un permiso
  if (!formData.permissions || formData.permissions === 0) {
    // Se puede reemplazar por un snackbar/Toast si existe en el proyecto
    alert('Seleccione al menos un permiso para el usuario.')
    return
  }

  // Emitir los datos del formulario para que el componente padre los procese
