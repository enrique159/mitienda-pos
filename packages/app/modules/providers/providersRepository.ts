...
export async function getProviders(): Promise<AppResponse<Provider[]>> {
  try {
    ...
    return response(
      false,
      'Error al traer los proveedores',
      normalizeError(err) as Provider[]
    )
  }
}

export async function getProvidesActive(): Promise<AppResponse<Provider[]>> {
  try {
...
export async function getProviderById(
  id: UUID
): Promise<AppResponse<Provider | null>> {
  try {
    const provider = await knex<Provider>('providers').where({ id }).first()
...
export async function deleteProvider(id: UUID): Promise<AppResponse<Provider>> {
  try {
    ...
    await knex<Provider>('providers').where({ id }).del()
