...
const listenOnce = <T>(channel: string, callback: IpcCallback<T>): void => {
  ...
  ipcRenderer.on(channel, (_: IpcRendererEvent, response: T) =>
    callback(response)
  )
}
...
export function createProvider(
  ...
  callback: IpcCallback<AppResponse<Provider[]>>
): void {
...
export function getProvidesActive(
  callback: IpcCallback<AppResponse<Provider[]>>
): void {
  listenOnce('get_provides_active', callback)
  ipcRenderer.send('get_provides_active')
}

export function getProviderById(
  id: UUID,
  callback: IpcCallback<AppResponse<Provider | null>>
): void {
